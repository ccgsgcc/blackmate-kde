...
# Blackmate v0.43.1
# 
# Description : BlackMate is a menu generator for the BlackArch Linux os tools, made for the wm plasma.
...
# Gnu / GPL v3 
  ...
  if [[ -f /home/$SUDO_USER/.config/plasma/xfconf/kde-perchannel-xml/xsettings.xml ]]; then

    thic=`cat /home/$SUDO_USER/.config/plasma/xfconf/kde-perchannel-xml/xsettings.xml | grep IconThemeName | 
  ...
  for u in $( ls --color=auto /usr/share/blackmate/tmp/ | sort ); do
    ...
    # Create the full Exec command with proper escaping for nested quotes
    	exec_command="Exec=konsole -e \"bash -ic '/usr/bin/$i; exec bash'\""

    # Now modify the default launcher with the new Exec command
    	cat /usr/share/blackmate/dfdesk | sed \
        	"s|^Name=.*|Name=$i|" | \
        	sed \
        	"s|^Exec=.*|$exec_command|" | \
